...
// Nguồn dữ liệu: data/topic-index.json — chỉ chứa CON TRỎ (examFile + questionId) tới các câu
// hỏi đã được admin gắn nhãn "chuong"/"dang" trong chính file đề gốc, KHÔNG có nội dung câu hỏi
// (giữ mục lục nhẹ). Khi bắt đầu ôn tập mới tải đúng những file đề thật sự có câu khớp bộ lọc.
...
async function init() {
  const [taxRes, topicRes] = await Promise.all([
    fetch("data/taxonomy.json?v=" + Date.now(), { cache: "no-store" }),
    fetch("data/topic-index.json?v=" + Date.now(), { cache: "no-store" })
  ]);
  taxonomy = await taxRes.json();
  topicIndex = await topicRes.json();
...
function onChuongChange() {
  ...
  [...qs("topic-list").children].forEach(card => {
    card.classList.toggle("selected", card.querySelector(".t-name").textContent === selectedChuong);
  });
...
function matchedQuestions() {
  const matches = currentMatches().filter(t => t.chuong === selectedChuong);
  if (selectedDang.size === 0) return matches; // không chọn dạng cụ thể = lấy hết dạng trong chương
...
async function startPractice() {
  ...
  try {
    // Chỉ tải đúng những file đề thật sự chứa câu khớp bộ lọc (không tải cả kho dữ liệu)
    const fileGroups = {};
    picked.forEach(p => {
    ...
    picked.forEach(p => {
      ...
      if (!q) return;
      // Đổi id để tránh trùng khi gộp câu hỏi từ nhiều đề khác nhau
    ...
    // Chỉ coi là "đáp án chuẩn" nếu TẤT CẢ các đề đóng góp câu hỏi đều là đáp án chuẩn
